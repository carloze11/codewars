...
function mixFruit(arr) {
    let regFruits = ["banana", "orange", "apple", "lemon", "grapes"];
    let specials = ["avocado", "strawberry", "mango"];
    let total = 0;

    arr.forEach((fruit) => {
        if (regFruits.includes(fruit.toLowerCase())) {
            total += 5;
        } else if (specials.includes(fruit.toLowerCase())) {
            total += 7;
        } else {
            total += 9;
        }
